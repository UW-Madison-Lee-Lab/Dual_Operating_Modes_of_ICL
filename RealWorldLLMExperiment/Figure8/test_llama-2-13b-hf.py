...
k_list = [0,1,2,4,8,16,32,64,128]



# Initialize the model ID
model_id = "meta-llama/Llama-2-13b-hf"
save_file_name = 'results/llama2-13b-hf'
tokenizer = LlamaTokenizer.from_pretrained(model_id)
model = LLM(model=model_id)#, enforce_eager=True, dtype=torch.float32)
'''
# Load the tokenizer
...
for index,data_name in enumerate(classification_datasets):
    ...
    for k in k_list: #num_examples    
        ...
        for random_seed in random_seed_list:
            ...
            print(gold_correct, rand_correct, count, int(gold_example/count), int(rand_example/count))
        ...
        D[k]['gold_example'] = gold_example/count
        D[k]['rand_example'] = rand_example/count
