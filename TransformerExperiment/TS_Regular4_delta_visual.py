...
parser.add_argument('--steps', default=10000, type=int, help='total number of training steps we want to run')#5000
...
if 1:
    ...
    #xx= [2,2,2,2,2]
    delta_m_list = xx
    delta_m_list = [1/args.b**a for a in delta_m_list]
    delta_w_list = xx
    delta_w_list = [1/args.b**a for a in delta_w_list]
    
    fig, axes = plt.subplots(args.epochs, 5, figsize=(22*2, 3.2*args.epochs*2), sharex=True, sharey=True)
    
    for col, (delta_str, delta_m, delta_w, x) in enumerate(zip(delta_strs, delta_m_list, delta_w_list, xx)):
        ...
        for epoch in range(1, args.epochs+1):
            ...
            if epoch == 1:
                ...
                axes[epoch-1,col].set_ylim([10**(-2.5),10**0])
                axes[epoch-1,col].set_yscale('log')
            
                axes[epoch-1,col].set_title(r'$\delta_{\mu}=\delta_{w}=$'+delta_str)
