...
if 1: # set 3x5 /pi, /
    resize = 6
    #fig = plt.figure(figsize=(10*resize, 6.5*resize))
    # Create a 3x4 grid using GridSpec
    #gs_main = gridspec.GridSpec(6, 5, figure=fig)
...
if 5: # draw 1x5 for distance of task learning 2
    row_index = 2
    for row, (sm, sw) in enumerate(zip(sm_list, sw_list)):
        folder = 'data/'+fig_name+'/sm='+str(sm)+' sw='+str(sw)+'/'
        print('\n***** ' +str(row)+ '-' +str(0)+ ' *****')
        ...
        if 1:
            ...
            ax.plot(means, label=r'$\|\tilde{{\boldsymbol{{w}}}} - \boldsymbol{{w}}^\alpha\|$', linewidth=linewidth, color='red')
        ...
        if 1:
            ...
            ax.plot(means, label=r'$\|\tilde{{\boldsymbol{{w}}}} - \boldsymbol{{w}}^*\|$', linewidth=linewidth, color='blue')
            ax.fill_between(range(len(means)), lower_bound, upper_bound, color='blue', alpha=0.15)
...
if 6: # draw 1x5 for prediction
    ...
    for row, (sm, sw) in enumerate(zip(sm_list, sw_list)):
        ...
        ax.plot(means, label=r'$(\mathcal{F}^* - y_{k+1}^{*})^2$', linewidth=linewidth, color='blue')
        ax.fill_between(range(len(means)), lower_bound, upper_bound, color='blue', alpha=0.15)
...
if 1:
    ...
    plt.tight_layout()
    plt.savefig('BayesianFig/'+fig_name+'.pdf',bbox_inches='tight')
